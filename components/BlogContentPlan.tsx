      ...
      <div className="bg-gradient-to-br from-violet-600/10 via-indigo-600/10 to-transparent border border-white/10 p-12 rounded-[40px] shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div className="space-y-2">
            ...
            <p className="text-slate-500 text-xs font-black uppercase tracking-widest-label">Master Strategic Strike V17.0</p>
          </div>
          <div className="flex items-center gap-3">
            <span className="px-6 py-2 rounded-2xl bg-white/5 border border-white/10 text-white text-[10px] font-black uppercase tracking-widest-label">
              30 Consecutive Strike Assets
            </span>
          </div>
        </div>
        <p className="text-slate-300 leading-relaxed text-lg max-w-4xl font-medium italic">
    ...
    <>
      <div className="bg-white/[0.02] border border-white/5 rounded-[32px] p-6 hover:border-violet-500/40 hover:bg-white/[0.04] transition-all duration-500 flex flex-col group relative backdrop-blur-md">
        <div className="flex justify-between items-center mb-6">
          <div className="bg-black/60 rounded-xl px-4 py-1.5 text-[10px] font-black text-violet-400 uppercase tracking-widest-label border border-violet-500/20">
            Phase {post.day.toString().padStart(2, '0')}
          </div>
        ...
        <div className="space-y-4 mb-6 flex-1">
          ...
          <div className="flex justify-between items-center bg-black/20 p-2.5 rounded-xl border border-white/5">
            <span className="text-[8px] text-slate-600 font-black uppercase tracking-widest">Mastery</span>
        ...
        <div className="mt-auto pt-4 border-t border-white/5">
          ...
          >
            ...
            {isWriting ? 'Modeling Draft...' : 'Synthesize 1.7K Word Deep-Dive'}
          </button>
        </div>
      </div>
      ...
      {showModal && fullPost && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-10 bg-slate-950/95 backdrop-blur-2xl animate-in fade-in duration-500">
          <div className="bg-[#0a0a0f] border border-white/10 w-full max-w-5xl max-h-full flex flex-col rounded-[50px] shadow-3xl overflow-hidden animate-in zoom-in-95 duration-500">
            <div className="p-10 border-b border-white/5 flex items-center justify-between bg-black/40">
              <div className="max-w-[85%]">
                ...
                <div className="flex items-center gap-4 mt-4">
                  <span className="px-3 py-1 bg-violet-500/10 border border-violet-500/20 text-violet-400 text-[9px] font-black uppercase tracking-widest-label rounded-lg">Master Recon V17.0</span>
                  <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest-label">{post.suggestedWordCount} Semantic Nodes Resolved</span>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-12 custom-scrollbar bg-slate-950/10">
              <article className="prose prose-invert lg:prose-2xl max-w-none prose-headings:font-display prose-headings:tracking-tighter prose-headings:uppercase prose-p:font-medium prose-p:text-slate-400">
            ...
            <div className="p-10 border-t border-white/5 bg-black/60 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                   <p className="text-xs text-white font-black uppercase tracking-widest-label">Master Strike Asset Verified</p>
                   <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">1,500+ Word Semantic Depth | 2025 Search System Compliant</p>
                </div>
              </div>
              <button
                onClick={() => {
                  navigator.clipboard.writeText(fullPost);
                  alert("Master intelligence asset copied to clipboard.");
                }}
                className="px-10 py-5 bg-white text-black hover:bg-slate-200 rounded-2xl font-black text-xs uppercase tracking-widest-label transition-all flex items-center gap-3 shadow-xl"
              >
                Copy Strike Asset
              </button>
            </div>
